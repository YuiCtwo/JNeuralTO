...
def curvature_loss(hessian_array):
    laplacian = hessian_array.sum(dim=-1).abs()
    laplacian[jt.isnan(laplacian)] = 0.0
    laplacian[jt.isneginf(laplacian)] = 0.0
    laplacian[jt.isposinf(laplacian)] = 0.0
    return laplacian.mean()


class NeuralTORunner:
    def __init__(self, mode="train", is_continue=False):
        ...
        self.use_mono_bkgd = self.cfg.use_mono_bkgd
        self.warm_up_end = self.cfg.warm_up_end
        self.anneal_end = self.cfg.anneal_end

        # background setting
        self.background_rgb = None
        if self.use_mono_bkgd:
            self.background_rgb = jt.zeros([1, 3])

        # mask settings
        # TODO: support mask loss
        self.use_mask = False
    ...
    def init_model(self):
        logging.info('Init model begin')
        self.geometry_network = build_from_cfg(self.cfg.model, NETWORKS)
        self.renderer = build_from_cfg(self.cfg.render, SAMPLERS)
        self.renderer.network_setup(self.geometry_network)
        logging.info('Init model end, using rendering model: {}'.format(str(self.renderer)))

    def init_dataset(self):
        logging.info('Init dataset begin, from: {}'.format(self.cfg.dataset["dataset_dir"]))
        dataset_cfg = deepcopy(self.cfg.dataset)
        dataset_cfg["d_type"] = "train"
        self.dataset_train = build_from_cfg(dataset_cfg, DATASETS)
        dataset_cfg["d_type"] = "val"
        self.dataset_val = build_from_cfg(dataset_cfg, DATASETS)

    def init_optimizer(self):
        # TODO: set optional different learning rate for different network
        self.optimizer = build_from_cfg(self.cfg.optim, OPTIMS, params=self.geometry_network.parameters())
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            data = self.dataset_train.gen_random_rays_at(image_perm[self.iter_step % len(image_perm)], self.batch_size)

            rays_o, rays_d, true_rgb, mask = data[:, :3], data[:, 3: 6], data[:, 6: 9], data[:, 9: 10]
            near, far = self.dataset_train.near_far_from_sphere(rays_o, rays_d)
            ...
            if "beta" in render_out.keys():
                # beta loss
                loss += render_out["beta"] * 0.08
            if "hessian" in render_out.keys():
                curved_loss = curvature_loss(render_out["hessian"])
                loss += curved_loss * 5e-5
    ...
    def get_image_perm(self):
        return jt.randperm(self.dataset_train.n_images)
    ...
    def load_checkpoint(self):
        ...
        model_list = []
        logging.info('Load checkpoint begin, from: {}'.format(os.path.join(self.base_exp_dir, 'checkpoints')))
        ...
        if latest_model_name is not None:
            ...
            checkpoint = jt.load(os.path.join(self.base_exp_dir, 'checkpoints', latest_model_name))
            self.geometry_network.load_state_dict(checkpoint['geometry'])
            # re-set iter_step
    ...
    def validate_image(self, idx=0, resolution_level=-1):
        if idx < 0:
            idx = np.random.randint(self.dataset_val.n_images)

        print('Validate: iter: {}, camera: {}'.format(self.iter_step, idx))

        if resolution_level < 0:
            resolution_level = self.validate_resolution_level
        rays_o, rays_d = self.dataset_val.gen_rays_at(idx, resolution_level=resolution_level)
        H, W, _ = rays_o.shape
        ...
        for rays_o_batch, rays_d_batch in zip(rays_o, rays_d):
            near, far = self.dataset_val.near_far_from_sphere(rays_o_batch, rays_d_batch)
        ...
        if len(out_normal_fine) > 0:
            ...
            rot = np.linalg.inv(self.dataset_val.pose_all[idx, :3, :3].detach().numpy())
            ...
            if len(out_rgb_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                           ...
                           np.concatenate([img_fine[..., i],
                                           self.dataset_val.image_at(idx, resolution_level=resolution_level)]))
            if len(out_normal_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                                        'normals',
                                        '{:0>8d}_{}_{}.png'.format(self.iter_step, i, idx)),
    ...
    def validate_mesh(self, world_space=False, resolution=64, threshold=0.0):
        ...
        bound_min = jt.float32(self.dataset_train.object_bbox_min)
        bound_max = jt.float32(self.dataset_train.object_bbox_max)
        ...
        if world_space:
            vertices = vertices * self.dataset_train.scale_mats_np[0][0, 0] + \
                       self.dataset_train.scale_mats_np[0][:3, 3][None]
